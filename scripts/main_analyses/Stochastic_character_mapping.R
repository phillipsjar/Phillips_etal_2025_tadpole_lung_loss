...
t_indep <- BayesTraits_simmap(master_indep, Nsim, data, model = "independent", 
                         trees, state_labels = four_state_state, 
                         trait_col_names = four_state_col)
t_dep <- BayesTraits_simmap(master_dep, Nsim, data, model = "dependent", 
                         trees, state_labels = four_state_state, 
                         trait_col_names = four_state_col)
t_six_indep <- BayesTraits_simmap(master_six_state_indep_export, Nsim, data, 
                         model = "six_state", trees, state_labels = six_state_state, 
                         trait_col_names = six_state_col)
t_six_dep <- BayesTraits_simmap(master_six_state_dep_export, Nsim, data, 
                         model = "six_state", trees, state_labels = six_state_state, 
                         trait_col_names = six_state_col)
...
avg_simmap_counts = function(simmap){
  ...
  counts <- apply(Y, c(1,2), mean, na.rm = TRUE)
