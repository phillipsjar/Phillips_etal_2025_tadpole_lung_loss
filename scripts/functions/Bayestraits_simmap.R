...
random_runs <- sample(1:(dim(BT_output)[1]), Nsim, replace = FALSE)  # get a sample of 100 random runs from the posterior
tree_numbers <- BT_output$Tree.No[random_runs]

rate_values <- vector(mode = "list", length = Nsim)             # empty list for the rate values for each random run
...
master_simmap <- vector(mode = "list", length = Nsim)             # empty list to be filled with the descendents of each node of ML tree
...
if(model == "dependent" | model == "independent"){
  ...
  print(Nsim-i) # count down for sanity
}

if(model == "six_state"){
  for(i in 1:Nsim){
      master_simmap[[i]] <- make.simmap(trees[[tree_numbers[i]]], sim_data, 
                          nsim=1, Q = custom_Q(rate_values[[i]], model = model, 
