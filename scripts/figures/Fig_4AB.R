...
{
BT_output <- read.csv("bayestraits/output/processed_logs/dep_allzero_2.txt", sep = "\t")
...
{BT_output2 <- read.csv("bayestraits/output/processed_logs/eight_state_res_1.txt", sep = "\t")
...
{pdf(file = "figures/Fig_4_A.pdf", width = 2.195, height = 2.6)
  ...
  vioplot(log(summary[,cols]*.1+1), xaxt = "n", yaxt = "n", colMed = "black",
          ylim = c(0,log(1+max(summary[,cols]))+.2))
...
vioplot(log(summary2[,cols2]+1), xaxt = "n", yaxt = "n", colMed = "black",
        ylim = c(0,log(1+max(summary2[,cols2]))+.2))
...
rescale(c(a[,1],b[,1]), to = c(.25, 12), from = range(a[,1]))
1.5/rescale(c(a[,1],b[,1]), to = c(.25, 12), from = range(a[,1]))*100
